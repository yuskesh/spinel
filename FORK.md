...
### Keeping it planted across upstream syncs

The 48 added lines have been byte-identical through every re-plant so far; only
where they sit moves. On the sync to upstream `53feb9df` neither `git apply` nor
`patch --fuzz=3` could place them, because upstream had inserted a new function
between the two anchors (removing the blank line that separated them) and
rewritten the prologue of `emit_method`. The patch was re-planted by hand at the
same three semantic positions — immediately before `emit_method_signature`, at
the top of its body, and at the top of `emit_method` — with the logic unchanged.

Verification each time is the same pair of checks: upstream's own test suite must
produce identical results with and without the patch, and the emitted C must be
byte-identical with the environment variable unset. For `53feb9df` that was 2587
pass / 0 fail / 7 error on both sides with the same seven non-passing tests, and
200 of upstream's own test programs compiled to identical C.
