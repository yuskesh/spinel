...
reject-test: $(SPINEL)
	...
	t=test/reject/typed_array_into_mutated_poly_param.rb; \
...
infer-test: $(SPINEL) $(SP_RT_LIB)
	...
	grep -q '^spinel: test/infer/why_widen_chain.rb:25:3: note: returned `return nil if n > 5…` is Integer, where a `return` gives nil (two kinds meet: untyped)' "$$tmp/wwc.err" && grep -q '^spinel: test/infer/why_widen_chain.rb:25:10: note: and `nil` is nil' "$$tmp/wwc.err" || { echo "infer-test: FAIL (--warn-widen: why: a return that met two kinds names the return of the other kind)"; cat "$$tmp/wwc.err"; ok=0; }; \
