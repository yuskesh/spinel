...
| `Fiber` body referencing a nested block param or a nested `Fiber.new`, and reassigning a captured heap object | a `Fiber.new` / `Enumerator.new` body that contains a nested block (`Fiber.new { 3.times { \|i\| ... } }`) or a nested `Fiber.new` does not compile; reassigning a captured String/Array (`s = "new"`, vs the supported in-place `s << x`) is not shared back | the body lowers to a flat C function, so a nested block's params and a nested fiber's body function have no place to live yet. Assigning an enclosing *value* local (Integer/Float/Boolean/poly) IS shared (heap cell), and in-place mutation of a captured heap object IS shared (by pointer) |
