...
| External `Enumerator` — `Enumerator::Lazy`, `Enumerator.new { \|y\| ... }`, and `.each` on an arbitrary method with no block | unsupported | a custom or lazy generator needs Fiber-style suspension. `Array#each` and `Range#each` with no block *do* return a working external Enumerator (`#next` / `#peek` / `#rewind` / `#size`, with `loop` stopping on `StopIteration`); chained block→`.to_a` forms (`each_slice(n).to_a`, `filter_map`, `map{}.to_a`) also work. |
