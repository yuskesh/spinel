...
| `Fiber` / proc capture written only inside a *nested block* | a capture that is assigned solely within a nested block in the body (`Fiber.new { 3.times { \|i\| acc += i } }`) is not shared back to the enclosing scope | capture detection does not descend into nested blocks, so the variable is treated as block-local. Affects every type (Integer as much as String). Assigning a capture *directly* in the body — including rebinding a String / Array / Hash / object to a new value — IS shared (value locals via a heap cell, heap objects via a typed-pointer cell), and in-place mutation of a captured heap object is shared by pointer |
