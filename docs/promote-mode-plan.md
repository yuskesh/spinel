...
- **残 ERR=44 / FAIL=22 の構造的ブロッカー**:
  ...
  3. **932→937 の安全グループ landed**: bound-method 直接 `.call` の arg を `emit_arg_or_default` で param 型 coerce(method/i960)/ ClassVariableWriteNode 推論を cvar 型返し=二重 box 解消(cvar_method_first_assign)/ poly-receiver dispatch の poly arg→scalar param unbox(8313, `<=>` 前進)/ splat 展開要素を poly param へ box(splat_call)。各 g_promote_mode gate 内、default 992/0/0+optcarrot 59662 維持。
  **残 ERR=37(937 時点、`/tmp/perr5.txt` ≈)の散在 one-off / 深い codegen バグ**:
