...
- **★次の大物 = ブロック反復 block-param(analyze レベル、codegen boxing では直せない)**: `[1,2,3,4].filter_map{|x| x*2 if x.even?}` 系。widen は block-param の scope local を poly 化するが、**最終 infer_type pass(analyze.c:2296 の全 node 再 infer)が typed-array 反復の block-param を element 型(int)に再導出して widen を打ち消す**。一方 `even?` 等一部の codegen 経路は scope local(poly)を読む → **同一 body 内で同じ `x` が int(`sp_int_mul`)と poly(`sp_poly_to_i`)に混在**し、反復束縛も `mrb_int lv_x__bp = Array_get(...)`(element 型 decl)が外側 `sp_RbVal lv_x__bp`(widen slot)を **shadow 二重宣言**。codegen で decl を poly 化すると body の `sp_int_mul(poly,..)` が新たに壊れる(=codegen 単独で不可)。**正解は analyze 側で block-param 型解決を widen と一貫させる**(typed-array 反復 param も promote で poly に固定、final infer で上書きさせない)。**inference 変更=default/optcarrot 回帰リスク**ゆえ専用に慎重に。each/map/select/filter_map/reduce 系の最大グループ(sample 10 中 4 がこの shadow pattern)。
- **次の一手(codegen 級)**: B4(builtin call-arg coerce、genuine な poly arg→int builtin param のみ)。`compile_typed_call_args` 相当の call-arg emission で poly arg→int param に `sp_poly_to_i`、float→`sp_poly_to_f`、bigint→`.v.p` を挿す中央 coerce。並行で rightward pattern(`expr=>a,b`=MatchRequiredNode 別経路)。手法=`/tmp/ptest.sh <name>`(単体 promote compile+diff)、`/tmp/perragg.sh`(全 ERR 並列 first-error 集計→`/tmp/perr_lines.txt`)。
