...
- **次の一手**: (1) B4 builtin call-arg の中央 coerce(poly arg→int param=`sp_poly_to_i`、float=`sp_poly_to_f`、bigint=`.v.p`)。(2) closure-capture front-end(capture 変数 poly 対応 or widen 除外)。(3) FAIL=20 の出力差(proc 等)。手法=`/tmp/ptest.sh <name>`、`/tmp/perragg.sh`→`/tmp/perr2.txt`、`/tmp/ptest_lastc` に最後の cfile。
