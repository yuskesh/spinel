...
/* Returns NULL for missing keys (Ruby `h[missing] == nil` semantics).
   Internal callers (values/dup/eq/...) read via h->order[i] which is
   always an existing key, so they never see the miss path. */
