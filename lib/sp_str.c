...
const char*sp_str_concat3(const char*a,const char*b,const char*c){SP_GC_ROOT(a);SP_GC_ROOT(b);SP_GC_ROOT(c);if(!a)a=sp_str_empty;if(!b)b=sp_str_empty;if(!c)c=sp_str_empty;size_t la=sp_str_byte_len(a),lb=sp_str_byte_len(b),lc=sp_str_byte_len(c);char*r=sp_str_alloc(la+lb+lc);memcpy(r,a,la);memcpy(r+la,b,lb);memcpy(r+la+lb,c,lc);return r;}
const char*sp_str_concat4(const char*a,const char*b,const char*c,const char*d){SP_GC_ROOT(a);SP_GC_ROOT(b);SP_GC_ROOT(c);SP_GC_ROOT(d);if(!a)a=sp_str_empty;if(!b)b=sp_str_empty;if(!c)c=sp_str_empty;if(!d)d=sp_str_empty;size_t la=sp_str_byte_len(a),lb=sp_str_byte_len(b),lc=sp_str_byte_len(c),ld=sp_str_byte_len(d);char*r=sp_str_alloc(la+lb+lc+ld);memcpy(r,a,la);memcpy(r+la,b,lb);memcpy(r+la+lb,c,lc);memcpy(r+la+lb+lc,d,ld);return r;}
...
sp_IntArray*sp_str_bytes(const char*s){SP_GC_ROOT(s);if(!s)sp_nil_recv("bytes");sp_IntArray*a=sp_IntArray_new();size_t n=sp_str_byte_len(s);for(size_t i=0;i<n;i++)sp_IntArray_push(a,(mrb_int)(unsigned char)s[i]);return a;}
const char *sp_str_crypt(const char *s, const char *salt) {SP_GC_ROOT(s);SP_GC_ROOT(salt);
  if (!salt) salt = sp_str_empty;
  char salt2[3];
...
sp_IntArray*sp_str_codepoints(const char*s){SP_GC_ROOT(s);if(!s)sp_nil_recv("codepoints");sp_IntArray*a=sp_IntArray_new();const char*p=s;while(*p){uint32_t cp;int n=sp_utf8_decode(p,&cp);sp_IntArray_push(a,(mrb_int)cp);p+=n;}return a;}
